...
#check if token exists, check expiration. If expired -> refresh_spotify_token
def spotify_authenticated(session_id):
    tokens = get_user_token(session_id)
    if tokens:
        expyr = tokens.expires_in
        if expyr <= timezone.now():
            refresh_spotify_token(session_id)
        return True
    return False

#Request API to refresh token and update user token.
